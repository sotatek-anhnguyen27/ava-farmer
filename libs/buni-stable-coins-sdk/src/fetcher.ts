...
import BuniCornFactory from './abis/BuniCornFactory.json'
import BuniCornPool from './abis/BuniCornPool.json'
...
export abstract class Fetcher {
  ...
  public static async fetchTokenData(
    chainId: ChainId,
    address: string,
    provider = getDefaultProvider(chainId === ChainId.BSC ? getNetwork(ChainId.MAINNET) : getNetwork(chainId)),
    symbol?: string,
    name?: string
  ): Promise<Token> {
   ...
   * @param factoryAddress address of BuniCornFactory
  ...
  public static async fetchPairData(
    ...
    provider = getDefaultProvider(tokenA.chainId === ChainId.BSC ? getNetwork(ChainId.MAINNET) : getNetwork(tokenA.chainId))
  ): Promise<Pair[]> {
    const addresses = await Fetcher.fetchPairAddresses(tokenA, tokenB, factoryAddress, provider)
    return Promise.all(
      addresses.map(async address => {
        let poolContract = new Contract(address, BuniCornPool.abi, provider)
  ...
  public static async fetchPairAddresses(
    ...
    if (typeof PAIR_ADDRESS_CACHE?.[chainId]?.[tokens[1].address]?.[tokenB.address] == 'undefined') {
      const factory = await new Contract(factoryAddress, BuniCornFactory.abi, provider)
      PAIR_ADDRESS_CACHE = {
        ...PAIR_ADDRESS_CACHE,
        [chainId]: {
