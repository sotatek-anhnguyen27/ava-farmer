...
import { BNB, Currency, ETHER } from './currency'
...
function wrappedAmount(currencyAmount: CurrencyAmount, chainId: ChainId): TokenAmount {
  ...
  if (currencyAmount.currency === ETHER || currencyAmount.currency === BNB) return new TokenAmount(WETH[chainId], currencyAmount.raw)
  invariant(false, 'CURRENCY')
}

function wrappedCurrency(currency: Currency, chainId: ChainId): Token {
  if (currency instanceof Token) return currency
  if (currency === ETHER || currency === BNB) return WETH[chainId]
  invariant(false, 'CURRENCY')
}
