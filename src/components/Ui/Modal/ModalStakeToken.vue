<template>
  <div>
    ...
    >
      <template #default>
        <div class="header" :class="_isThemeGame && 'game-header'">
          {{ actionType }} {{ isSinglePool ? singleToken : $t('lpTokens') }}
        </div>
        <div class="content">
          ...
          >
            ...
            <div class="text-label-right">
              ...
              >
                {{ isSinglePool ? singleToken : $t('lp') }}
              </div>
            </div>
          </div>
          <div :class="_isThemeGame && 'game-text'" style="margin-top:11px;">
            <div v-if="error" class="error" :class="_isThemeGame && 'game-error'">
              {{ error.text }}
              <!-- <a
                v-show="error.type !== 2 && !isStackedBuni"
                :href="liquidityUrl"
                target="_blank"
                class="error url"
          ...
          <div v-if="isGameNFT(pid) && actionType === 'Unstake' && currentLevelName !== 'Bunicorn Logo'">
            <MessageError
              v-if="true"
              :text="$t('ifYouRemoveLiquidity', { currentLevelName })"
              class="mt-4"
            />
          </div>
          <div v-if="!isGameNFT(pid) && actionType === 'Unstake' && currentLevelName !== 'Bunicorn Logo'">
            <MessageError
              v-if="true"
              :text="$t('ifYouRemoveLiquidity2', { currentLevelName })"
              class="mt-4"
            />
          </div>
    ...
    <ModalWarningUnstakeFarm
      :imgUrlCurLevel="currentUrlImage"
      :open="openWarningUnstakeFarm"
      @closeModal="$emit('closeStakeModal')"
      @confirmUnstake="onUnStake"
...
import ModalWarningUnstakeFarm from '@/components/Ui/Modal/ModalWarningUnstakeFarm';
import config from '@/config';
import abi from '@/helpers/abi';

export default {
  components: { CurrencyInput, ModalWarningUnstakeFarm },
  data() {
    return {
      amount: '',
      message: '',
      isLoading: false,
      openWarningUnstakeFarm: false,
      // timeConfig: 0,
      // lastTimeDeposit: 0
    };
  },
  ...
  computed: {
    ...
    displayToken() {
      return this.fullToken != 0 && this.fullToken < 1e-4
        ? '<0.0001'
  ...
  methods: {
    ...
    async onStake() {
      ...
      try {
        ...
        this.$store.dispatch('farm/getFarmData');
      } catch (e) {
        console.error(
    ...
    async confirmUnStake() {
      if (!this.isGameNFT(this.pid)) {
        return await this.onUnStake();
      }
      if (this.currentLevelName !== 'Bunicorn Logo') {
        this.openWarningUnstakeFarm = true;
        return;
      }
