<template>
  <div class="farm-row">
    <div class="row-info" @click="toggleDetail">
      ...
      <div class="farm-column farm-info text-left mobile-show hidden">
        <div class="label-row" :class="_isThemeGame ? 'game' : ''">
          {{ $t('apr') }}
        </div>
      ...
      <div class="farm-column farm-info text-left hide hidden">
        <div
          class="label-row"
      ...
      <div class="farm-column farm-info text-left hide">
        ...
        <div class="content">
           {{ remainBlockNumber > 0 ? remainBlockNumber : 0 | formatBlock }} blocks
        </div>
      </div>
    ...
    <div v-show="openDetail" class="detail" :class="_isThemeGame ? 'game' : ''">
      <div class="detail-infos text-left hide link-and-cardgame">
        <div class="link-info" v-if="!isSinglePool">
          <div>
            <a
    ...
    <portal to="modal">
      <ModalROI
        ...
        :buniPrice="rewardTokenPrice"
        :apy="originalValue"
        :liquidityUrl="`${poolLink}?addLiquidity=true`"
        :earnedType="rewardToken.symbol"
        :isSinglePool="isSinglePool"
      />
    </portal>
  </div>
...
<script>
import StakedColumn from '@/components/Farms/HeroArena/StakedColumn';
...
export default {
  ...
  computed: {
    ...
    isSinglePool() {
      return this.poolTokens.length === 1;
    },
    ...
    earningsBusd() {
      return this.buniEarned
        ? new BigNumber(this.buniEarned)
            .multipliedBy(this.rewardTokenPrice)
            .toNumber()
        : 0;
    },
    account() {
    ...
    poolLink() {
      ...
      if (this.details.poolLink) {
        return this.details.poolLink;
      }
      return getPoolLink(this.details.lpAddresses, 'tokens');
    }
  },
  methods: {
    toggleDetail() {
    ...
    async onReward() {
      ...
      try {
        await Helper.stakeSyrupFarm(
          provider,
          this.details.poolAddress,
          0,
          true
        );
