    ...
    <div class="unlock-wrapper" :class="_isThemeGame && 'game-unlock-wrap'">
      ...
      <template v-else-if="isApproved">
        ...
        >
          <UiLoading v-if="buttonLoadingStatus.stake" />
          <span v-else>
            {{ isSinglePool ? `Stake ${farmData.lpSymbol}` : $t('stakeLP') }}
          </span>
        </button>
      </template>
      <template v-else>
...
import ModalStakeToken from '@/components/Farms/HeroArena/ModalStakeToken';
...
export default {
  ...
  computed: {
    ...
    allowance() {
      ...
      console.log(
        'tokenAllowance',
        new BigNumber(tokenAllowance[this.farmData.lpAddresses] || 0).toString()
      );
  ...
  methods: {
    ...
    async handleClick() {
      ...
      console.log('FARM DATA', this.farmData);
      try {
