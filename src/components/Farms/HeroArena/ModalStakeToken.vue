    ...
    >
      <template #default>
        <div class="header" :class="_isThemeGame && 'game-header'">
          {{ actionType }} {{ isTopSinglePool ? label : $t('lpTokens') }}
        </div>
        <div class="content">
          ...
          <div :class="_isThemeGame && 'game-text'" style="margin-top:11px;">
            ...
            <warning
              v-if="actionType !== 'Unstake' && tax > 0"
              :text="
                `You will be charged ${tax}% tax when staking into this pool. ${$t(
                  'stakeHeroArenaWarning'
                )}`
              "
              class="mb-4 mt-4"
            />
          ...
          <div v-if="actionType === 'Unstake'">
            <MessageError
              v-if="true"
              :text="$t('unstakeHeroArenaWarning')"
              class="mt-4"
            />
          </div>
...
import Warning from '@/components/Message/Warning.vue';

export default {
  components: { CurrencyInput, Warning },
  data() {
    return {
      amount: '',
      message: '',
