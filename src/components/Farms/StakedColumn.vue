    ...
    <div class="unlock-wrapper" :class="_isThemeGame && 'game-unlock-wrap'">
        ...
        <div class="onPrensent" :class="_isThemeGame ? 'game-btn-input-2' : ''">
          ...
          <div class="stake-btn-wrapper">
            <button
              ...
              :disabled="
                buttonLoadingStatus.unstake ||
                  isLoadingFarmData ||
                  $store.state.account.wrongNetwork
              "
            >
      ...
      <template v-else-if="isApproved">
        ...
        >
          <UiLoading v-if="buttonLoadingStatus.stake" />
          <span v-else>
            {{ isSinglePool ? $t(`Stake ${singleToken}`) : $t('stakeLP') }}
          </span>
        </button>
      </template>
      <template v-else>
        <button
          ...
          v-if="!isApproved"
          :disabled="
            buttonLoadingStatus.enable_farm ||
              isLoadingFarmData ||
              $store.state.account.wrongNetwork
          "
          :class="`${_isThemeGame ? 'game-border-btn-enable' : ''}`"
...
import { getMasterChefAddress, getBalanceNumber } from '@/helpers/farm';
import ModalStakeToken from '@/components/Ui/Modal/ModalStakeToken';
  ...
  computed: {
    ...
    allowance() {
      ...
      const tokenAllowance =
        allowances[`${this.farmData.poolAddress}-${this.farmData.pid}`];
    ...
    displayBalance() {
      ...
      return this.fullToken != 0 && this.rawStakedBalance <= 1e-4
        ? '<0.0001'
        : new BigNumber(rawStakedBalance).toString();
    },
    rawStakedBalance() {
      const farm = this.$store.state.farm.data.find(f => f.pid === this.pid);
      ...
      if (this.isApproved) {
        if (this.rawStakedBalance) {
          return this.isSinglePool
            ? `${this.singleToken} ${this.$t('Staked')}`
            : `LP ${this.$t('Staked')}`;
        }
        return this.isSinglePool
          ? `${this.$t('stake')} ${this.singleToken}`
          : `${this.$t('stake')} LP`;
      }
    ...
    async handleClick() {
      ...
      try {
        const masterChefContract = new Contract(
          getMasterChefAddress(this.farmData.pid),
          abi['MasterChef'],
          provider.getSigner()
        );
        ...
        this.$store.dispatch('account/getFarmAllowances');
