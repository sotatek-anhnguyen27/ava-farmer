...
const mutations = {
  ...
  GET_BUNI_TOKENS_SUMMARY_REQUEST() {
    console.debug('GET_BUNI_TOKENS_SUMMARY_REQUEST');
  },
  GET_BUNI_TOKENS_SUMMARY_SUCCESS() {
    console.debug('GET_BUNI_TOKENS_SUMMARY_SUCCESS');
  },
  GET_BUNI_TOKENS_SUMMARY_FAILURE(_state, payload) {
    console.debug('GET_BUNI_TOKENS_SUMMARY_FAILURE', payload);
  },
...
const actions = {
  ...
  getBuniTokensSummary: async ({ commit }) => {
    commit('GET_BUNI_TOKENS_SUMMARY_REQUEST');
    try {
      const { buni } = await request('getBuni', {});
      return buni;
      commit('GET_BUNI_TOKENS_SUMMARY_SUCCESS');
    } catch (e) {
      commit('GET_BUNI_TOKENS_SUMMARY_FAILURE', e);
    }
  },
  ...
  getUserPoolShares: async ({ commit, rootState }) => {
    const address = rootState.account.address;
